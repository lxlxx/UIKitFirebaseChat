...
//    func fetchChatPartnerMessages_B(){
//        guard let uid = FIRAuth.auth()?.currentUser?.uid, let toId = currentUser?.id else {
//            return
//        }
//        
//        let userMessagesRef = FIRDatabase.database().reference().child("user_Messages").child(uid).child(toId)
...
//                }
//                
//                self.currentMessages.append(textMessage(dict:dictionary))
//                DispatchQueue.main.async(execute: {
//                    self.collectionView?.reloadData()
//                    //scroll to the last index
//                    let indexPath = IndexPath(item: self.currentMessages.count - 1, section: 0)
...
// MARK: - Collection View data source
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        switch currentMessages[indexPath.item].messageType {
        case .text:
